...
def build_facts_block(packet: dict[str, Any]) -> str:
    ...
    return (
        ...
        "or matchup that decided the game. Use the press conference for at most two or "
        "three direct quotes if they are clearly attributable, otherwise paraphrase. "
        "Use the highlights only as atmosphere. Close on a concrete stat or a quote, "
        "not a cliche. Remember: no em dashes, straight quotes only, and every quoted "
        "span must be verbatim from the corrected press-conference transcript.\n\n"
